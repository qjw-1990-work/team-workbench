...
function normalizeData(d) {
  ...
  // 客户细分配置：{ clientId: ['细分1', '细分2', ...] }
  if (!d.clientSegments) d.clientSegments = JSON.parse(JSON.stringify(defaultClientSegments));
  return d;
}
...
function getDefaultColumnWidths() {
  return [
    ...
    { id: 'col_title', label: '任务内容', width: '240px', min: '120px', max: '600px' },
    ...
    { id: 'col_progress', label: '当前进展', width: '120px', min: '80px', max: '300px' },
...
// 客户细分配置：{ clientId: ['细分1', '细分2', ...] }
const defaultClientSegments = {
  'c1': ['上海HELLA', '长春HELLA'],
  'c8': ['长春欧摩威', '墨西哥欧摩威'],
  'c10': ['长春Merit', '墨西哥Merit'],
};
...
// ==================== 视图切换 ====================
const viewTitles = { kanban: '任务看板', team: '团队成员', stats: '数据统计', notifications: '通知中心', permissions: '权限设置', clientSegments: '客户细分管理' };
...
function switchView(viewName) {
  ...
  if (viewName === 'clientSegments') renderClientSegments();
}
...
function getTaskStatus(task) {
  ...
  // 待办逻辑：
  // - 创建者给自己建任务（创建者 ∈ 责任人）→ 不显示待办，直接按时间逻辑
  // - 创建者派发任务给别人（创建者 ∉ 责任人）→ 大家都看到待办，直到责任人点击后切换
  var isSelfAssigned = task.createdBy && task.assignees &&
    task.assignees.includes(task.createdBy);

  if (!isSelfAssigned && task.createdBy && task.assignees && task.assignees.length > 0) {
...
function getSegmentOptions(clientId) {
  ...
  if (!client) return [];
  // 优先使用管理员配置的客户细分
  if (data.clientSegments && data.clientSegments[clientId] && data.clientSegments[clientId].length > 0) {
    return data.clientSegments[clientId];
  }
  ...
  // 长春Merit/墨西哥Merit → 长春Merit, 墨西哥Merit
  if (name === '长春Merit') return ['长春Merit', '墨西哥Merit'];
  // 其它客户 → 客户细分 = 客户名称
  return [name];
}
...
function updatePermissionUI() {
  ...
  // 客户细分管理入口仅管理员可见
  show('navClientSegments', isAdmin());
}
...
function attachColumnResizeHandles() {
  if (!isAdmin()) return; // 仅管理员可拖拽调整列宽
  document.querySelectorAll('.task-list-header').forEach(function(header) {
    // 移除旧手柄
    header.querySelectorAll('.col-resize-handle').forEach(function(h) { h.remove(); });
    var cols = header.querySelectorAll(':scope > div');
    cols.forEach(function(col, idx) {
      if (idx >= cols.length - 1) return; // 跳过最后一列（操作列）
      // 确保父元素可以定位
...
function startColumnResize(e) {
  e.preventDefault();
  var colIdx = parseInt(e.target.dataset.colIdx);
  var header = e.target.closest('.task-list-header');
  var col = e.target.parentElement;
  var cw = data.columnWidths || getDefaultColumnWidths();
  var curWidth = parseInt(cw[colIdx].width);
  if (isNaN(curWidth)) curWidth = col.offsetWidth;
...
function onColumnResizeMove(e) {
  ...
  var newWidth = Math.max(30, resizeState.startWidth + delta);
  var cw = data.columnWidths || getDefaultColumnWidths();
  cw[resizeState.colIdx].width = newWidth + 'px';

  // 实时更新所有表头和行
  var grid = getColumnWidthsGrid();
  document.querySelectorAll('.task-list-header, .task-row').forEach(function(el) {
    el.style.gridTemplateColumns = grid;
  });
}

function onColumnResizeUp(e) {
...
// ==================== 客户细分管理面板（仅管理员） ====================
function renderClientSegments() {
  var container = document.getElementById('clientSegmentsContainer');
  if (!container) return;
  if (!data.clientSegments) data.clientSegments = JSON.parse(JSON.stringify(defaultClientSegments));

  var html = '<div class="perm-group"><div class="perm-group-header"><span>🏷️</span> 客户细分选项管理</div><div class="perm-group-body">';
  html += '<p style="color:var(--muted);font-size:12px;margin-bottom:12px">为每个客户配置创建任务时的"客户细分"下拉选项，用逗号分隔多个细分（如：长春Merit, 墨西哥Merit）</p>';

  data.clients.forEach(function(client) {
    var segs = (data.clientSegments[client.id] || []).join(', ');
    html += '<div class="perm-row" style="padding:12px 16px">';
    html += '<div style="flex:1">';
    html += '<div class="perm-row-label">' + escapeHtml(client.name) + '</div>';
    html += '<div class="perm-row-desc" style="margin-top:4px">';
    html += '<input type="text" class="form-input" data-client-id="' + client.id + '" value="' + escapeHtml(segs) + '" placeholder="如：长春Merit, 墨西哥Merit" style="width:100%;font-size:13px;padding:6px 10px;border:1px solid var(--rule);border-radius:var(--radius-sm)" onchange="updateClientSegments(\'' + client.id + '\', this.value)">';
    html += '</div>';
    html += '</div></div>';
  });

  // 为两个Merit客户端添加快捷设置
  html += '<div style="padding:8px 16px;border-top:1px solid var(--rule);margin-top:8px">';
  html += '<span style="font-size:12px;color:var(--muted);margin-right:8px">快捷设置：</span>';
  html += '<button class="btn btn-secondary" style="font-size:11px;padding:4px 10px" onclick="quickSetMeritSegments()">为Merit客户端设置"长春Merit, 墨西哥Merit"</button>';
  html += '</div>';
...
function updateClientSegments(clientId, value) {
  var segs = value.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
  if (segs.length === 0) {
    delete data.clientSegments[clientId];
  } else {
    data.clientSegments[clientId] = segs;
  }
  saveData();
  showToast('客户细分已更新', 'success');
}

function quickSetMeritSegments() {
  data.clientSegments['c10'] = ['长春Merit', '墨西哥Merit'];
  data.clientSegments['c14'] = ['长春Merit', '墨西哥Merit'];
  saveData();
  renderClientSegments();
  showToast('Merit客户端细分已设置', 'success');
}
